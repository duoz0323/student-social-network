...
/** Chứng minh Nearby kế thừa JWT, account ACTIVE và onboarding guard chung. */
@WebMvcTest(NearbyDiscoveryController.class)
...
class NearbyDiscoverySecurityTest {
    ...
    @MockitoBean private NearbyDiscoveryService service;
    ...
    @Test
    void blockedAccountIsRejectedByJwtFilter() throws Exception {
        User blocked = user(8L, UserRole.USER, UserStatus.BLOCKED);
        authenticate("blocked", blocked, true);
        mockMvc.perform(request().header("Authorization", "Bearer blocked"))
    ...
    void incompleteProfileReturnsApprovedErrorBeforeService() throws Exception {
        ...
        mockMvc.perform(request().header("Authorization", "Bearer incomplete"))
                .andExpect(status().isForbidden())
    ...
    void eligibleUserCanAccess() throws Exception {
        ...
        mockMvc.perform(request().header("Authorization", "Bearer eligible"))
                .andExpect(status().isOk());
    }

    private org.springframework.test.web.servlet.request.MockHttpServletRequestBuilder request() {
        return get("/api/v1/discovery/nearby")
    ...
    private User user(Long id, UserRole role, UserStatus status) {
        User user = new User("nearby-security-" + id + "@example.com", "hash");
