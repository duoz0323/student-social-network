...
@WebMvcTest(AdminHashtagController.class)
...
class AdminHashtagSecurityTest {
    ...
    @MockitoBean private AdminHashtagService service;
    ...
    void activeUserReturns403() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/admin/hashtags").header("Authorization", "Bearer user"))
    ...
    void activeAdminCanAccess() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/admin/hashtags").header("Authorization", "Bearer admin"))
                .andExpect(status().isOk());
    }

    @Test
    void blockedAdminIsRejected() throws Exception {
        authenticate("blocked", user(1L, UserRole.ADMIN, UserStatus.BLOCKED));
        mockMvc.perform(get("/api/v1/admin/hashtags").header("Authorization", "Bearer blocked"))
    ...
    private User user(Long id, UserRole role, UserStatus status) {
        User user = new User("hashtag" + id + "@example.com", "hash");
