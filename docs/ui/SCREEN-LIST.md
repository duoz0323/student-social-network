...
- ADMIN-01, ADMIN-02, ADMIN-03, ADMIN-04, ADMIN-05, ADMIN-06, ADMIN-07, ADMIN-08, ADMIN-09.
...
| AUTH-03 | Onboarding tên và username | `docs/ui/screens/auth/update-profile-1.jpg` | User chưa hoàn tất hồ sơ | Nhập tên hiển thị và username bắt buộc sau đăng ký. | Username có prefix `@` ở UI, kiểm tra availability có debounce; request không chứa `@`. | Route `/onboarding/profile`, bước 1/3. | Phạm vi triển khai: MVP_CURRENT. Backend luôn validate lại format, reserved và unique khi hoàn tất. |
...
| PROFILE-03 | Modal chỉnh sửa hồ sơ | `docs/ui/screens/profile/model-edit-profile.jpg` | User | Cập nhật hồ sơ cá nhân. | Avatar, tên hiển thị, bio/thông tin cá nhân, ngày sinh bắt buộc, lưu; username chỉ đọc. | Mở từ PROFILE-01. | Phạm vi triển khai: MVP_CURRENT. Không cho xóa ngày sinh hoặc lưu khi người dùng chưa đủ 18 tuổi; chưa triển khai đổi username hoặc hồ sơ riêng tư. |
