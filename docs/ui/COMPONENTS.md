...
| `AdminShell` | Khung trang quản trị gồm sidebar admin và content rộng. | ADMIN-01 đến ADMIN-13. | `activeNav`, `adminUser`, `children`. | Dùng chung admin. |
| `AdminSidebar` | Điều hướng quản trị. | ADMIN-01 đến ADMIN-13. | `activeItem`, `onBackToApp`. | Dùng chung admin. |
...
| Component | Trách nhiệm | Màn hình sử dụng | Dữ liệu/props dự kiến | Phạm vi |
|---|---|---|---|---|
| `PostCard` | Hiển thị bài viết và Location tùy chọn trong feed, profile, saved, liked và search. | FEED-01, PROFILE-01/02, POST-07, LIKED-01, SEARCH-01 nếu có kết quả bài. | `post` gồm `location` object hoặc `null`; `locationMeta` là metadata trình bày tạm thời như khoảng cách Nearby. | Dùng chung module post/feed/profile; không đưa distance vào Post response chung. |
...
| `NearbyDiscovery` | Điều phối state Geolocation, radius, page đầu, lỗi và danh sách Nearby. | FEED-01 tab Gần bạn. | `active`; dùng `useNearbyDiscovery`, không nhận/lưu tọa độ ngoài runtime. | Module feed; không ảnh hưởng For You/Following. |
| `NearbyControls` / `NearbyPostList` | Chọn radius, cập nhật vị trí và render PostCard kèm khoảng cách bằng cursor Infinite Scroll. | FEED-01 tab Gần bạn. | `radiusKm`, callbacks location/load-more, `posts`, pagination state. | Responsive, keyboard accessible; invalid cursor không retry. |
| `DiscoveryMap` / `useDiscoveryMap` | Điều phối viewport dirty, marker request, Location Posts cursor, GPS click-only và chống response cũ. | FEED-01 chế độ Bản đồ. | `active`; state cục bộ, AbortController/request ID riêng cho marker và panel. | Không lưu tọa độ, không gọi API theo pan/zoom và không dùng global store. |
| `DiscoveryMapCanvas` | Khởi tạo Google Map từ loader dùng chung với Places, render marker, user marker và cluster. | FEED-01 chế độ Bản đồ. | `locations`, `selectedLocationId`, `userCoordinates`, callbacks viewport/marker. | Cluster click chỉ fit bounds; SDK lỗi có fallback/retry; API key lấy từ env. |
| `DiscoveryMapPanel` | Hiển thị Location, `postCount` và PostCard theo cursor; desktop side panel/mobile bottom sheet. | FEED-01 chế độ Bản đồ. | Selected Location và các state loading/success/empty/error/loading-more/end. | Đổi marker reset cursor/list; khử trùng Post ID; dùng PostCard chuẩn. |
...
| `StudentRecommendationRail` | Hiển thị 4 gợi ý ban đầu trong card cố định bên phải trên các trang Feed ở desktop rộng. | FEED-01. | Tự tải page đầu, “Xem thêm” mở thêm tối đa 3 người rồi đổi thành “Thu gọn”, semantic reason, profile navigation và Follow. | Danh sách cuộn ẩn scrollbar trong chiều cao card cố định; dùng icon kết nối người dùng, căn mép trên với Feed, giữ nút Follow trên một dòng và không hiển thị `matchScore`. |
...
| `MessageThread` | Render text/ảnh/shared-post bubble, trạng thái sending/failed/seen và retry. | MSG-02. | `messages`, current user, read marker. | Không render HTML động. |
| `SharedPostMessage` | Render snapshot bài theo quyền viewer hoặc trạng thái bài không còn khả dụng; click sang chi tiết bài. | MSG-02. | `sharedPost`, `unavailable`. | Không dùng snapshot được gửi từ client. |
| `SharePostDialog` | Modal chọn một recipient, tìm kiếm/phân trang, lời nhắn tùy chọn, gửi DM, copy link và Facebook share. | Feed, profile, post detail. | `post`, open/close. | Chỉ tạo/reuse conversation khi bấm Gửi; không dùng Facebook token. |
...
| `AdminAcademicPage` | Điều phối tab Academic/Interest, drill-down School → Faculty → Major, tìm kiếm, phân trang và trạng thái tải/lỗi/rỗng. | ADMIN-10. | `keyword`, `page`, `size`, selected School/Faculty; API qua `adminApi`. | Dùng `DataTable`, không gọi Axios trực tiếp và không hiển thị thao tác hard delete. |
| `AdminAcademicFormDialog` | Form dùng chung cho create/edit School, Faculty, Major và Interest; School có shortName tùy chọn. | ADMIN-10. | `kind`, `item`, `submitting`, `error`, callbacks. | Backend chuẩn hóa/kiểm tra duplicate cuối cùng. |
| `AdminAcademicStatusDialog` | Xác nhận đổi ACTIVE/INACTIVE và giải thích bảo toàn reference/không cascade. | ADMIN-10. | `kind`, `target`, `submitting`, `error`, callbacks. | Bắt buộc xác nhận khi chuyển ACTIVE → INACTIVE. |
| `AdminManagementPage` | Danh sách tài khoản ADMIN và modal chi tiết theo permission. | ADMIN-11. | `admins`, `selectedAdmin`, `roles`, `permissions`; API qua `adminApi`. | Click item mở chi tiết; Master Admin chỉ đọc, không hiện thao tác role/cấp lại mật khẩu/vô hiệu hóa; Admin hỗ trợ vẫn được quản lý theo permission. |
| `AdminRolePermissionsPage` | Tạo role tùy chỉnh và cấu hình ma trận permission. | ADMIN-11. | `roles`, `permissions`, `selectedRoleCode`, form `name`; API qua `adminApi`. | Chỉ SUPER_ADMIN Bootstrap; ba permission phân quyền bị khóa ở role khác; role mới có Tổng quan mặc định. |
| `AdminProfilePage` | Hồ sơ tự quản lý và đổi mật khẩu của ADMIN. | ADMIN-12. | `profile`, `draft`, `password`; API qua `adminApi`. | Thông tin quyền chỉ đọc; đổi mật khẩu thành công đăng xuất toàn bộ phiên. |
