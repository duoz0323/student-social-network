...
export default function PostComposer({ open, onClose }) {
  ...
  const customHeader = (
    <header className="shrink-0 relative flex items-center justify-between border-b border-[var(--app-border)] px-5 py-4">
      <button onClick={handleClose} className="text-[15px] text-[var(--app-muted)] transition hover:text-[var(--app-text)]">Hủy</button>
      ...
      footer={
        <div className="flex w-full items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button disabled={(!content.trim() && !imageUrl) || content.length > 500} onClick={submit} className="!rounded-full px-6 py-2 min-h-[40px] font-bold bg-[var(--app-text)] text-[var(--app-surface)] hover:bg-[var(--app-text)]/90 disabled:opacity-30 disabled:bg-[var(--app-border)] disabled:text-[var(--app-muted)] border-none">
              Đăng
            </Button>
          </div>
        </div>
