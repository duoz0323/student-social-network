...
export default function CollaboratorPostAnalyticsPage() {
  ...
  return <section className="space-y-6"><AdminPageHeader icon={ChartNoAxesCombined} title="Thống kê bài viết" description={state.data.post.contentPreview || `Bài #${postId}`} actions={<select value={range} onChange={(e) => setRange(e.target.value)} className="h-11 rounded-xl border border-zinc-300 bg-white px-4 text-sm font-medium outline-none transition focus:border-zinc-900 focus:ring-2 focus:ring-zinc-100"><option value="24H">24 giờ</option><option value="7D">7 ngày</option><option value="30D">30 ngày</option></select>} />
